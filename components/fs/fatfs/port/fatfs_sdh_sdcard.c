...
{
	return SDH_ReadMultiBlocks(buff, sector, gSDCardInfo.blockSize, count);
...
int MMC_disk_write(const BYTE *buff, LBA_t sector, UINT count, unsigned char async_finish)
{
    status_t ret;

_retry:
    ret = SDH_WriteMultiBlocks((uint8_t *)buff, sector, gSDCardInfo.blockSize, count, async_finish);
