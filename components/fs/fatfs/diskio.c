...
)
{
    ...
    // if (pdrv > (DEV_NUM_MAX*2)+1) {

    
    unsigned char async_finish = (pdrv & 0b10000000)/0b10000000;
    pdrv &= ~0b10000000;

    if (pdrv > DEV_NUM_MAX) {
        return STA_NOINIT;
    }
    // // add DEV_NUM_MAX+1 to the value to make it async
    // unsigned char async_finish = ((pdrv/(DEV_NUM_MAX+1)) >= 1) ? 1 : 0;
    // pdrv = pdrv % (DEV_NUM_MAX+1);


    if (pDiskioDriver[pdrv].disk_write) {
        result = pDiskioDriver[pdrv].disk_write(buff, sector, count, async_finish);
    }
