...
status_t SDH_WriteMultiBlocks(uint8_t *writebuff, uint32_t WriteAddr, uint16_t BlockSize, uint32_t NumberOfBlocks, bool async_finish)
{
    status_t errorstatus = Status_Success;
...
if (!async_finish) {
...
#else
    // printf("Real_int_clear_shouldnt_be_ran");
    uint32_t intFlag;
    while (1) { //this loop will block until the read is finished
        intFlag = SDH_GetIntStatus();
        if (intFlag & SDH_INT_DATA_ERRORS || intFlag & SDH_INT_DMA_ERROR || intFlag & SDH_INT_AUTO_CMD12_ERROR) {
            sd_status = SD_DataCfg_ERROR;
            break;
    ...
    // printf("Real_int_clear_shouldnt_be_ran 2");
    SDH_ClearIntStatus(intFlag); //I think this resets the register flag
    // while (true) {} 
...
// int SDH_WriteMultiBlocks_async_finish(void) {
//     status_t errorstatus = Status_Success;
//     SD_Error sd_status;
//     uint32_t time_node;
...
/*
* @brief Clear takes options; 0 - dont clear; 1 - clear when done; 2 - clear instantly (return)
* @brief Returns 0 - data writen; 1 - no updates; 2 - error 
*/
int SDH_WriteMultiBlocks_async_check(int clear) {
    
    uint32_t intFlag;
    SD_Error sd_status;
    intFlag = SDH_GetIntStatus();
    // printf("fake,good,flag: %d\r\n", intFlag);
    if (clear == 2) {
        SDH_ClearIntStatus(intFlag);
        return 1;
    }
...
int SDH_WriteMultiBlocks_async_finish(void) {
    ...
    if (sd_async_write_in_progress == 1 && SDH_WriteMultiBlocks_async_check(0) != 0) {
		...
		if (ret == 2 || (bflb_mtimer_get_time_ms() - start_time > 1000)) {
            //err status will never be droped :| not sure if thats fine
			printf("Diskio timeout :|");
			return 1;
		}
	}
	if (sd_async_write_in_progress == 1) {
		SDH_WriteMultiBlocks_async_check(2);
        ret = WaitInProgramming();
    }
